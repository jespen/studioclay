...
export async function GET(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' && process.env.NEXT_PUBLIC_SWISH_TEST_MODE !== 'true') {
      logDebug('Setting up Swish certificates from environment variables in test-course endpoint');
      const certSetupResult = setupCertificate();
      
      if (!certSetupResult.success) {
        logError('Failed to set up Swish certificates in test-course endpoint:', certSetupResult);
